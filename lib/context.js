...
// options: .api, .forCache, .responseOnly
export function fetchRawInfrastructure(scriptPath, options) {
    ...
    const injectedFunctions = {
        ...
        getCachedJSON(providerId) {
            if (!options.api) {
                throw "api";
            }
            const url = `${options.api.base}/api-cache/${options.api.version}/providers/${providerId}/fetch.json`;
            const result = getResult(url);
            const json = JSON.parse(result.data);
            return {
                response: json,
